...
class ToolsPage(BasePage):
    ...
    def _click_and_wait_for_tool_fetch(self, button: Locator, modal_id: str) -> None:
        ...
        with self.page.expect_response(
            lambda resp: (
                re.search(r"/v1/admin/tools/[0-9a-f]", resp.url) is not None and "/partial" not in resp.url and "/search" not in resp.url and "/ids" not in resp.url and resp.request.method == "GET"
            ),
            timeout=30000,
        ) as response_info:
