...
async def require_admin_auth(
    ...
    if getattr(settings, "email_auth_enabled", False):
        try:
                    ...
                    if username:
                        ...
                        if current_user and current_user.is_admin:
                            ...
                            if "text/html" in accept_header:
                                ...
                                raise HTTPException(status_code=status.HTTP_302_FOUND, detail="Admin privileges required", headers={"Location": f"{root_path}/v1/admin/login?error=admin_required"})
                            else:
            ...
            if "text/html" in accept_header:
                root_path = resolve_root_path(request)
                raise HTTPException(status_code=status.HTTP_302_FOUND, detail="Authentication required", headers={"Location": f"{root_path}/v1/admin/login"})
